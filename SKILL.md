...
Check if the diff touches frontend files 检测前端文件变更:

```bash
# 检测 diff 是否涉及前端文件
SCOPE_FRONTEND=$(git diff origin/<base> --name-only 2>/dev/null | grep -qE "\.(css|scss|less|html|erb|haml|slim|jsx|tsx|vue|svelte)$" && echo "true" || echo "false"); echo "SCOPE_FRONTEND=$SCOPE_FRONTEND"  # 检测前端文件范围 <base> 2>/dev/null)
```
...
2. **Read `design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
